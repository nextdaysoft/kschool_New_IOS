...
class NumbersNamesShowVC: BaseViewController {
    ...
    var passedWordPairs: [NumbersNames] = []
    var passedCorrectMap: [String: String] = [:]
    ...
    override func viewDidLoad() {
        ...
        setupWordsRandomly(data: passedWordPairs)
        levelLabel.text = levelText.localiz()
        setupCorners()
    }
    ...
    func setupWordsRandomly(data: [NumbersNames]) {

        let shuffledFirst = data.shuffled()
        let shuffledLast  = data.shuffled()
    ...
    func calculatePercentage() -> Int {
        ...
        for (firstLabel, lastLabel) in userMatches {
            ...
            let expectedID = passedCorrectMap[firstID] ?? ""
    ...
    @IBAction func submitAndNextTapBtn(_ sender: UIButton) {
        ...
        if isSubmitted {
            ...
            // ✅ SAVE DATA
            UserDefaults.standard.set(true, forKey: "numbersNamesLevel\(levelNumber)Completed")
            UserDefaults.standard.set(percentage, forKey: "numbersNamesLevel\(levelNumber)Percentage")

            // ✅ POP TO MENU
            if let menuVC = navigationController?.viewControllers.first(
                where: { $0 is NumbersNamesMenuVC }
            ) {

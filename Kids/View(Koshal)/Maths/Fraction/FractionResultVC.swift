...
class FractionResultVC: BaseViewController {
    ...
    func goToMenu() {
        ...
        // ✅ Save globally
//        UserDefaults.standard.set(percentage, forKey: "compareNumbersPercentage")
        UserDefaults.standard.set(true, forKey: "mathsLevel17Completed")
        UserDefaults.standard.set(percentage, forKey: "mathsLevel17Percentage")

        if let mathsVC = navigationController?.viewControllers.first(
            where: { $0 is MathsMenuVC }
        ) as? MathsMenuVC {

            // ✅ Direct UI update (instant)
            mathsVC.updateLevel(
                level: 17,
            ...
            print("❌ MathsMenuVC not found in stack")
        }
    }

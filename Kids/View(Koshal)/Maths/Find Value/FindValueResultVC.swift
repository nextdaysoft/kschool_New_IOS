...
class FindValueResultVC: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        for i in 0..<results.count {
            ...
            labels[i]?.text = "\(result.questionNumber) -> \(result.correctAnswer)"
    ...
    func goToMenu() {
        ...
        // ✅ FIXED (level 12)
        UserDefaults.standard.set(true, forKey: "mathsLevel12Completed")
        UserDefaults.standard.set(percentage, forKey: "mathsLevel12Percentage")

        if let menuVC = navigationController?.viewControllers.first(where: { $0 is MathsMenuVC }) {

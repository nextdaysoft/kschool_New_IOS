...
class CalculateMoneyResultVC: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        for i in 0..<results.count {
            ...
            labels[i]?.text = result.questionText
    ...
    func goToMenu() {
        ...
        // 🔥 LEVEL BASED SAVE
        let completedKey = "calculateMoneyLevel\(levelNumber)Completed"
        let percentageKey = "calculateMoneyLevel\(levelNumber)Percentage"

        UserDefaults.standard.set(true, forKey: completedKey)
        UserDefaults.standard.set(percentage, forKey: percentageKey)

        // 🔥 BACK TO MENU
        if let menuVC = navigationController?.viewControllers.first(where: { $0 is CalculateMoneyMenuVC }) {

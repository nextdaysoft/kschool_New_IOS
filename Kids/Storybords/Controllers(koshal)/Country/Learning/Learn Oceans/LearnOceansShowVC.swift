//
//  LearnOceansShowVC.swift
//  KSchool
//
//  Created by Koshal Singh on 30/03/26.
//

import UIKit
import AVFoundation

class LearnOceansShowVC: BaseViewController,AVSpeechSynthesizerDelegate {
    ...
    var planetsN: [LearnPlanetsItem] = [
        .init(text: "ArcticOcean - the smallest Arctic ocean", imageName: "ocean_arctic"),
        .init(text: "Southern Ocean (also called Antarctic Ocean)", imageName: "ocean_southern"),
        .init(text: "Atlantic Ocean", imageName: "ocean_atlantic"),
        .init(text: "Pacific Ocean - the largest Arctic ocean", imageName: "ocean_pacific"),
        .init(text: "Indian Ocean", imageName: "ocean_indian"),
    ]
    ...
    func showSwipeUpHint() {
        ...
        let label = UILabel()
        label.text = "Swipe up to change oceans"
    ...
    func animateCardOut() {
        ...
        let currentItem = planetsN[currentIndex]
        ...
        UIView.animate(withDuration: 0.4, animations: {
            ...
            if self.currentIndex >= self.planetsN.count {
    ...
    func restoreCardFromSmall(cardTag: Int) {
        ...
        // ✅ Already visited cards same rahenge
        let viewedItems = Array(
            planetsN.prefix(currentIndex + 1)
        )

        // ✅ Sirf aage wale items reshuffle
        var remainingItems = Array(
            planetsN.dropFirst(currentIndex + 1)
        )

        remainingItems.shuffle()

        planetsN = viewedItems + remainingItems
    ...
    func addCardToDropView(with item: LearnPlanetsItem) {
        ...
        // ❌ NO background (gap ka main reason)
        smallCard.backgroundColor = .clear

        smallCard.layer.cornerRadius = imgBGView.layer.cornerRadius
        smallCard.layer.borderWidth = imgBGView.layer.borderWidth
        smallCard.layer.borderColor = imgBGView.layer.borderColor
        ...
        // 🔥 IMAGE — FULL EDGE TO EDGE (0 GAP)
        let imageView = UIImageView(frame: smallCard.bounds)
        imageView.image = imgView.image
        imageView.contentMode = .scaleAspectFill
        imageView.clipsToBounds = true

        // 🔥 LABEL — DIRECT OVER IMAGE (NO SPACE PUSH)
        let titleLabel = UILabel()
        titleLabel.frame = CGRect(x: 0, y: 0, width: cardSize, height: 30)
        ...
        // transparent bg (so image dikhe)
        titleLabel.backgroundColor = UIColor.white.withAlphaComponent(0.6)
    ...
    func updateUI() {
        ...
        guard currentIndex < planetsN.count else { return }

        let item = planetsN[currentIndex]
    ...
    func goToResultScreen() {
        ...
        if let resultVC = storyboard.instantiateViewController(
            withIdentifier: "LearnContinentsResultVC"
        ) as? LearnContinentsResultVC {
    ...
    func createFruitsPDF() -> URL? {
        ...
        let pdfURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("Oceans.pdf")
        ...
        do {
            ...
            try renderer.writePDF(to: pdfURL) { context in
                ...
                for (index, item) in planetsN.enumerated() {
    ...
    @IBAction func repeatTapBtn(_ sender: UIButton) {
        speak(text: planetsN[currentIndex].text)
    }

//
//  GuessTheCountryCapitalsResultVC.swift
//  KSchool
//
...
class GuessTheCountryCapitalsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheCountryCapitalsLevel\(levelNumber)Completed"
            let percentageKey = "guessTheCountryCapitalsLevel\(levelNumber)Percentage"

            var levelData = UserDefaults.standard.dictionary(forKey: "guessTheCountryCapitalsLevelData") as? [String: Int] ?? [:]

            levelData["\(levelNumber)"] = percentage

            UserDefaults.standard.set(levelData, forKey: "guessTheCountryCapitalsLevelData")
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheCountryCapitalsMenuVC }
        ) {

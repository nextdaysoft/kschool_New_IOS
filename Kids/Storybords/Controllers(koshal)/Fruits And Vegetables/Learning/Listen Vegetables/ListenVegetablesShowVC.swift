...
class ListenVegetablesShowVC: BaseViewController,AVSpeechSynthesizerDelegate {
    ...
    func showSwipeUpHint() {
        ...
        let label = UILabel()
        label.text = "Swipe up to change vegetables"
    ...
    func restoreCardFromSmall(cardTag: Int) {
        ...
        guard cardTag < planets.count else { return }

        currentIndex = cardTag

        // Already viewed items same rahenge
        let viewedItems = Array(planets[0...currentIndex])

        // Aage ke items dobara random
        ...
        imgBGView.transform = CGAffineTransform(
            scaleX: 0.7,
            y: 0.7
        )
    ...
    func goToResultScreen() {
        ...
        if let resultVC = storyboard.instantiateViewController(
            withIdentifier: "ListenVegetablesResultVC"
        ) as? ListenVegetablesResultVC {
    ...
    func createFruitsPDF() -> URL? {
        ...
        let pdfURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("Vegetables.pdf")

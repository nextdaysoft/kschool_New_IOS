//
//  ListenVegetablesResultVC.swift
//  Kids
//
...
class ListenVegetablesResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let completedKey = "listenVegetablesLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: completedKey)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is ListenVegetablesMenuVC }
        ) {

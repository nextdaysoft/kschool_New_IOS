//
//  GuessTheVegetablesResultVC.swift
//  Kids
//
...
class GuessTheVegetablesResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheVegetablesLevel\(levelNumber)Completed"
            let percentageKey = "guessTheVegetablesLevel\(levelNumber)Percentage"

            var levelData = UserDefaults.standard.dictionary(forKey: "guessTheVegetablesData") as? [String: Int] ?? [:]

            levelData["\(levelNumber)"] = percentage

            UserDefaults.standard.set(levelData, forKey: "guessTheVegetablesData")
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheVegetablesMenuVC }
        ) {

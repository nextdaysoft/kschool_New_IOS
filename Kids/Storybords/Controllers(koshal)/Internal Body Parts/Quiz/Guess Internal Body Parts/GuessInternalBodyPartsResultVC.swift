//
//  GuessInternalBodyPartsResultVC.swift
//  KSchool
//
...
class GuessInternalBodyPartsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            // ✅ Correct keys
            UserDefaults.standard.set(true, forKey: "internalBodyPartsLevel2Completed")
            UserDefaults.standard.set(percentage, forKey: "internalBodyPartsLevel2Percentage")
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is InternalBodyPartsMenuVC }
        ) {

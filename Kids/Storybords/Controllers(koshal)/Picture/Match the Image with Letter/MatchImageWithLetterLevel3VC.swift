//
//  MatchImageWithLetterLevel3VC.swift
//  Kids
//
...
class MatchImageWithLetterLevel3VC: BaseViewController {
    ...
    // MARK: - Setup Methods
    func setupLabels() {
        let letters = ["K", "L", "M", "N", "O"].shuffled()
    ...
    @IBAction func submitAndNextTapBtn(_ sender: UIButton) {
        ...
        if isSubmitted {
            ...
            // 🔐 SAVE MATCH IMAGE WITH LETTER – LEVEL 3
            UserDefaults.standard.set(true, forKey: "matchImageLetterLevel3Completed")
            UserDefaults.standard.set(percentage, forKey: "matchImageLetterLevel3Percentage")
        ...
        // 🔽 aapka existing validation / coloring logic (same rahega)
        for (label, imageView) in userMatches {
            let letter = label.text ?? ""

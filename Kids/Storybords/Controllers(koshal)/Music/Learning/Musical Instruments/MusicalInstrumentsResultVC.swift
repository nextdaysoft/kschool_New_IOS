//
//  MusicalInstrumentsResultVC.swift
//  KSchool
//
...
class MusicalInstrumentsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let completedKey = "musicalInstrumentsLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: completedKey)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is MusicalInstrumentsMenuVC }
        ) {

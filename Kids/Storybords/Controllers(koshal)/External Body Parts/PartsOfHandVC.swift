...
class PartsOfHandVC: BaseViewController {

    @IBOutlet weak var littleFingerView: UIView!
    @IBOutlet weak var elbowView: UIView!
    @IBOutlet weak var wristView: UIView!
    @IBOutlet weak var ringFingerView: UIView!
    @IBOutlet weak var middleFingerView: UIView!
    @IBOutlet weak var upperArmView: UIView!
    @IBOutlet weak var thumbView: UIView!
    @IBOutlet weak var indexFingerView: UIView!
    @IBOutlet weak var palmView: UIView!
    @IBOutlet weak var forearmView: UIView!
    ...
    override func viewDidLoad() {
        ...
        labels.forEach { label in
            label?.font = UIFont.systemFont(ofSize: 15, weight: .medium)   // Size change karo
        ...
        setupView(littleFingerView)
        setupView(elbowView)
        setupView(wristView)
        setupView(ringFingerView)
        setupView(middleFingerView)
        setupView(upperArmView)
        setupView(thumbView)
        setupView(indexFingerView)
        setupView(palmView)
        setupView(forearmView)
        ...
        label9.tag = gameLabelTag
        label10.tag = gameLabelTag
        
        
        correctMapping = [
            littleFingerView: "Little Finger",
            elbowView: "Elbow",
            wristView: "Wrist",
            ringFingerView: "Ring Finger",
            middleFingerView: "Middle Finger",
            upperArmView: "Upper Arm",
            thumbView: "Thumb",
            indexFingerView: "Index Finger",
            palmView: "Palm",
            forearmView: "Forearm"
        ]
    ...
    func setupRandomLabels() {
        ...
        let parts = ["Little Finger", "Elbow", "Wrist", "Ring Finger", "Middle Finger", "Upper Arm", "Thumb", "Index Finger", "Palm", "Forearm"]
    ...
    func checkDrop(_ draggedView: UIView) {
        ...
        let targets: [UIView: UIView] = [
            view1: littleFingerView,
            view2: elbowView,
            view3: wristView,
            view4: ringFingerView,
            view5: middleFingerView,
            view6: upperArmView,
            view7: thumbView,
            view8: indexFingerView,
            view9: palmView,
            view10: palmView,
    ...
    @IBAction func nextAndUndoTapBtn(_ sender: UIButton) {
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is ExternalBodyPartsMenuVC }
        ) as? ExternalBodyPartsMenuVC {

            UserDefaults.standard.set(true, forKey: "externalBodyPartsLevel3Completed")
            UserDefaults.standard.set(percentage, forKey: "externalBodyPartsLevel3Percentage")

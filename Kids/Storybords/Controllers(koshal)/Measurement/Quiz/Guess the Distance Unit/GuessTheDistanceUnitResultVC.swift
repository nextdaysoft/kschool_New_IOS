//
//  GuessTheDistanceUnitResultVC.swift
//  KSchool
//
...
class GuessTheDistanceUnitResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            UserDefaults.standard.set(true, forKey: "measurementLevel9Completed")
            UserDefaults.standard.set(percentage, forKey: "measurementLevel9Percentage")
        }
        ...
        if let menuVC = navigationController?.viewControllers.first(
            ...
            menuVC.level9Percentage = percentage

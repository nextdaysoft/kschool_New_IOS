//
//  GuessTheWeightUnitResultVC.swift
//  KSchool
//
...
class GuessTheWeightUnitResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            UserDefaults.standard.set(true, forKey: "measurementLevel8Completed")
            UserDefaults.standard.set(percentage, forKey: "measurementLevel8Percentage")
        }
        ...
        if let menuVC = navigationController?.viewControllers.first(
            ...
            // 👇 ONLY level8 pass
            menuVC.level8Percentage = percentage

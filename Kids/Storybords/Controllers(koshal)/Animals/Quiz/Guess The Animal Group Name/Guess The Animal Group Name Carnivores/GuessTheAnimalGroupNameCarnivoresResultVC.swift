//
//  GuessTheAnimalGroupNameCarnivoresResultVC.swift
//  Kids
//
...
class GuessTheAnimalGroupNameCarnivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalGroupNameCarnivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalGroupNameCarnivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalGroupNameCarnivoresMenuVC }
        ) {

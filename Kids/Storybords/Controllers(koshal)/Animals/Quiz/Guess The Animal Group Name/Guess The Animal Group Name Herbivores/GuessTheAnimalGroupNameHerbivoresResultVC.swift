//
//  GuessTheAnimalGroupNameHerbivoresResultVC.swift
//  Kids
//
...
class GuessTheAnimalGroupNameHerbivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalGroupNameHerbivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalGroupNameHerbivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalGroupNameHerbivoresMenuVC }
        ) {

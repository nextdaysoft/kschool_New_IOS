//
//  GuessTheAnimalGroupNameBirdsResultVC.swift
//  Kids
//
...
class GuessTheAnimalGroupNameBirdsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalGroupNameBirdsLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalGroupNameBirdsLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalGroupNameBirdsMenuVC }
        ) {

//
//  GuessTheAnimalGroupNameOmnivoresResultVC.swift
//  Kids
//
...
class GuessTheAnimalGroupNameOmnivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalGroupNameOmnivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalGroupNameOmnivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalGroupNameOmnivoresMenuVC }
        ) {

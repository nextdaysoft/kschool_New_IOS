//
//  GuessTheAnimalSoundResultVC.swift
//  Kids
//
...
class GuessTheAnimalSoundResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalSoundLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalSoundLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalSoundLevelMenuVC }
        ) {

//
//  GuessTheAnimalSemiAquaticResultVC.swift
//  Kids
//
...
class GuessTheAnimalSemiAquaticResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalSemiAquaticLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalSemiAquaticLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalSemiAquaticMenuVC }
        ) {

//
//  GuessTheAnimalBirdsResultVC.swift
//  Kids
//
...
class GuessTheAnimalBirdsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalBirdsLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalBirdsLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalBirdsMenuVC }
        ) {

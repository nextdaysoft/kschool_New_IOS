//
//  GuessTheAnimalAquaticResultVC.swift
//  Kids
//
...
class GuessTheAnimalAquaticResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalAquaticLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalAquaticLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalAquaticMenuVC }
        ) {

//
//  GuessTheAnimalOmnivoresResultVC.swift
//  Kids
//
...
class GuessTheAnimalOmnivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalOmnivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalOmnivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalOmnivoresMenuVC }
        ) {

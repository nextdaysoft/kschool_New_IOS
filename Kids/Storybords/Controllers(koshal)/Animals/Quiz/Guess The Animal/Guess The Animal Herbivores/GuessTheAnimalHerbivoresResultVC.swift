//
//  GuessTheAnimalHerbivoresResultVC.swift
//  Kids
//
...
class GuessTheAnimalHerbivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalHerbivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalHerbivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalHerbivoresMenuVC }
        ) {

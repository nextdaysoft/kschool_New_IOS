//
//  GuessTheAnimalCarnivoresResultVC.swift
//  Kids
//
...
class GuessTheAnimalCarnivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheAnimalCarnivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheAnimalCarnivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheAnimalCarnivoresMenuVC }
        ) {

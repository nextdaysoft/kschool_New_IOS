//
//  GuessTheBabyAnimalOmnivoresResultVC.swift
//  Kids
//
...
class GuessTheBabyAnimalOmnivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheBabyAnimalOmnivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheBabyAnimalOmnivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheBabyAnimalOmnivoresMenuVC }
        ) {

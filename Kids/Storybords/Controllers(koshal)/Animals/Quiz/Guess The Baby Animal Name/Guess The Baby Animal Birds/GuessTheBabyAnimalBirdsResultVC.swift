//
//  GuessTheBabyAnimalBirdsResultVC.swift
//  Kids
//
...
class GuessTheBabyAnimalBirdsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheBabyAnimalLevel\(levelNumber)Completed"
            let percentageKey = "guessTheBabyAnimalLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheBabyAnimalBirdsMenuVC }
        ) {

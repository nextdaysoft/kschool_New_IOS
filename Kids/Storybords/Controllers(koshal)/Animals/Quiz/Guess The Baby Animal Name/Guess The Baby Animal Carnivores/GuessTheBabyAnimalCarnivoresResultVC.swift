//
//  GuessTheBabyAnimalCarnivoresResultVC.swift
//  Kids
//
...
class GuessTheBabyAnimalCarnivoresResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheBabyAnimalCarnivoresLevel\(levelNumber)Completed"
            let percentageKey = "guessTheBabyAnimalCarnivoresLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheBabyAnimalCarnivoresMenuVC }
        ) {

//
//  LearnBirdsAnimalGroupNameResultVC.swift
//  Kids
//
...
class LearnBirdsAnimalGroupNameResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let key = "learnBirdsAnimalGroupNameLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: key)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnBirdsAnimalGroupNameMenuVC }
        ) {

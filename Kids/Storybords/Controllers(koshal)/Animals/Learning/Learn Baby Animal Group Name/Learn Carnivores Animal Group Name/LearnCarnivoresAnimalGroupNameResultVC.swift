//
//  LearnCarnivoresAnimalGroupNameResultVC.swift
//  Kids
//
...
class LearnCarnivoresAnimalGroupNameResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let key = "learnCarnivoresGroupNameLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: key)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnCarnivoresAnimalGroupNameMenuVC }
        ) {

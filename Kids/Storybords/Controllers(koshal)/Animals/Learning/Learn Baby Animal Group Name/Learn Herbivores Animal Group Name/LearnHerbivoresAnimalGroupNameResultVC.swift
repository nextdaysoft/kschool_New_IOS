//
//  LearnHerbivoresAnimalGroupNameResultVC.swift
//  Kids
//
...
class LearnHerbivoresAnimalGroupNameResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let key = "learnHerbivoresGroupNameLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: key)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnHerbivoresAnimalGroupNameMenuVC }
        ) {

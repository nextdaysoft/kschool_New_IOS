//
//  LearnOmnivoresAnimalGroupNameResultVC.swift
//  Kids
//
...
class LearnOmnivoresAnimalGroupNameResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let key = "learnOmnivoresGroupNameLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: key)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnOmnivoresAnimalGroupNameMenuVC }
        ) {

//
//  OmnivoresBabyNamesResultLevel1VC.swift
//  Kids
//
...
class OmnivoresBabyNamesResultLevel1VC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let key = "omnivoresBabyNamesLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: key)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is OmnivoresBabyNamesLevelMenuVC }
        ) {

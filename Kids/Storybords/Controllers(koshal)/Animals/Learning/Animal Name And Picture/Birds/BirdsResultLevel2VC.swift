...
class BirdsResultLevel2VC: BaseViewController {
    ...
    func goToMenu() {
        ...
        UserDefaults.standard.set(true, forKey: "birdsLevel2Completed")
            if let menuVC = navigationController?.viewControllers.first(
                where: { $0 is BirdsLevelMenuVC }
            ) as? BirdsLevelMenuVC {

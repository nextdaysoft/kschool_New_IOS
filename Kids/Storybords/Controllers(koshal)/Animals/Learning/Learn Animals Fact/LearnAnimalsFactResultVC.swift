//
//  LearnAnimalsFactResultVC.swift
//  Kids
//
...
class LearnAnimalsFactResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let key = "learnAnimalsFactLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: key)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnAnimalsFactLevelMenuVC }
        ) {

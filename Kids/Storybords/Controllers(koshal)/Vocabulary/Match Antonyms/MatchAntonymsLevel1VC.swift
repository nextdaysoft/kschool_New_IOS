//
//  MatchAntonymsLevel1VC.swift
//  Kids
//
//  Created by Koshal Singh on 31/01/26.
//
import LanguageManager_iOS
struct WordPairForAntonym {
    let first: String
    let last: String
}
...
class MatchAntonymsLevel1VC: BaseViewController {
    ...
    var correctWordMap: [String: String] = [
        "Up".localiz(): "Down".localiz(),
        "Hot".localiz(): "Cold".localiz(),
        "Big".localiz(): "Small".localiz(),
        "Happy".localiz(): "Sad".localiz(),
        "Day".localiz(): "Night".localiz()
    ]

    let wordPairs: [WordPairForAntonym] = [
        WordPairForAntonym(first: "Up".localiz(), last: "Down".localiz()),
        WordPairForAntonym(first: "Hot".localiz(), last: "Cold".localiz()),
        WordPairForAntonym(first: "Big".localiz(), last: "Small".localiz()),
        WordPairForAntonym(first: "Happy".localiz(), last: "Sad".localiz()),
        WordPairForAntonym(first: "Day".localiz(), last: "Night".localiz())
    ]
    ...
    @IBAction func submitAndNextTapBtn(_ sender: UIButton) {
        ...
        if isSubmitted {
            ...
            // ✅ SAVE DATA
            UserDefaults.standard.set(true, forKey: "matchAntonymsLevel1Completed")
            UserDefaults.standard.set(percentage, forKey: "matchAntonymsLevel1Percentage")

            // ✅ POP TO MENU
            if let menuVC = navigationController?.viewControllers.first(
                where: { $0 is MatchAntonymsLevelMenuVC }
            ) {
    ...
    func createPDF() -> URL? {
        ...
        let pdfURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("MatchAntonyms.pdf")

//
//  PickTheCorrectSpellingResultLevel10VC.swift
//  Kids
//
...
class PickTheCorrectSpellingResultLevel10VC: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        for i in 0..<results.count {
            ...
            if result.isCorrect {
                ...
                labels[i]?.text = "\(result.selectedWord), correct is \(result.correctWord)"
    ...
    func goToMenu() {
        ...
        UserDefaults.standard.set(true, forKey: "pickCorrectSpellingLevel10Completed")
        UserDefaults.standard.set(percentage, forKey: "pickCorrectSpellingLevel10Percentage")

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is PickTheCorrectSpellingMenuVC }
        ) {

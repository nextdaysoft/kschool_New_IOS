//
//  HealthyFoodHabitsResultLevel1VC.swift
//  Kids
//
...
class HealthyFoodHabitsResultLevel1VC: BaseViewController {
    ...
    func goToMenu() {
        ...
        let percentage = (finalScore * 100) / totalQuestions

        UserDefaults.standard.set(true, forKey: "healthyFoodLevel1Completed")
        UserDefaults.standard.set(percentage, forKey: "healthyFoodLevel1Percentage")

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is HealthyFoodHabitsMenuVC }

...
class GuessTheColorResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheColorLevel\(levelNumber)Completed"
            let percentageKey = "guessTheColorLevel\(levelNumber)Percentage"

            var levelData = UserDefaults.standard.dictionary(forKey: "guessTheColorLevelData") as? [String: Int] ?? [:]

            levelData["\(levelNumber)"] = percentage

            UserDefaults.standard.set(levelData, forKey: "guessTheColorLevelData")
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheColorMenuVC }
        ) {

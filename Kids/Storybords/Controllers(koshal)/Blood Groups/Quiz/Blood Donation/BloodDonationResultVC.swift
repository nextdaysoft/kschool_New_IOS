...
class BloodDonationResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            UserDefaults.standard.set(true, forKey: "learnBloodGroupsLevel2Completed")
            UserDefaults.standard.set(percentage, forKey: "learnBloodGroupsLevel2Percentage")
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnBloodGroupsMenuVC }
        ) {

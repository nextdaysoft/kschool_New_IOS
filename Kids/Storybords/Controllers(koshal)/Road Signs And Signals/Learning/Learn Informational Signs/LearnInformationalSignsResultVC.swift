//
//  LearnInformationalSignsResultVC.swift
//  KSchool
//
...
class LearnInformationalSignsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let completedKey = "learnInformationalSignsLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: completedKey)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnInformationalSignsMenuVC }
        ) {

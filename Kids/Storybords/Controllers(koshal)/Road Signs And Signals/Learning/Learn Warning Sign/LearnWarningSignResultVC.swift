//
//  LearnWarningSignResultVC.swift
//  KSchool
//
...
class LearnWarningSignResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let completedKey = "learnWarningSignLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: completedKey)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnWarningSignMenuVC }
        ) {

//
//  LearnRegulatorySignsResultVC.swift
//  KSchool
//
...
class LearnRegulatorySignsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            let completedKey = "learnRegulatorySignsLevel\(levelNumber)Completed"
            UserDefaults.standard.set(true, forKey: completedKey)
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is LearnRegulatorySignsMenuVC }
        ) {

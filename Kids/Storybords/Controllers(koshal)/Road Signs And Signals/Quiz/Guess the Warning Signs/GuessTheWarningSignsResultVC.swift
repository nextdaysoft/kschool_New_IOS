//
//  GuessTheWarningSignsResultVC.swift
//  KSchool
//
...
class GuessTheWarningSignsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "guessTheWarningSignsLevel\(levelNumber)Completed"
            let percentageKey = "guessTheWarningSignsLevel\(levelNumber)Percentage"
            ...
            UserDefaults.standard.set(levelData, forKey: "guessTheWarningSignsLevelData")
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is GuessTheWarningSignsMenuVC }
        ) {

//
//  GuessAllRoadSignsResultVC.swift
//  KSchool
//
...
class GuessAllRoadSignsResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            // ✅ Correct keys
            UserDefaults.standard.set(true, forKey: "roadSignsAndSignalsLevel7Completed")
            UserDefaults.standard.set(percentage, forKey: "roadSignsAndSignalsLevel7Percentage")
        }

        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is RoadSignsAndSignalsMenuVC }
        ) {

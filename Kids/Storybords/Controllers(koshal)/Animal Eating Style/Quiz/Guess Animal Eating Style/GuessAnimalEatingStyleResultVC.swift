//
//  GuessAnimalEatingStyleResultVC.swift
//  KSchool
//
...
class GuessAnimalEatingStyleResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "animalEatingStyleLevel\(levelNumber)Completed"
            let percentageKey = "animalEatingStyleLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is AnimalEatingStyleMenuVC }
        ) {

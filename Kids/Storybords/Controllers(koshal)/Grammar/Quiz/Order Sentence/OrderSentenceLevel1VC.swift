...
class OrderSentenceLevel1VC: BaseViewController {
    ...
    let sentencePairs: [(String, String)] = [
        ("Keep".localiz(), "quiet".localiz()),
        ("Drink".localiz(), "water".localiz()),
        ("Stand".localiz(), "up".localiz()),
        ("Open".localiz(), "book".localiz()),
        ("Wash".localiz(), "hands".localiz()),
        ("Eat".localiz(), "food".localiz()),
        ("Play".localiz(), "ball".localiz()),
        ("Close".localiz(), "door".localiz()),
        ("Sit".localiz(), "down".localiz()),
        ("Read".localiz(), "story".localiz())
    ]

    var currentIndex = 0
    var correctSentence: (String, String) = ("", "")
    ...
    func loadCurrentWord() {
        guard currentIndex < sentencePairs.count else {
            goToResultScreen()
            return
        }
        ...
        correctSentence = sentencePairs[currentIndex]
    ...
    func setupLetters() {
        let words = [correctSentence.0, correctSentence.1].shuffled()
    ...
    func checkResult() {
        ...
        if isCorrect {
            ...
            results.append(
                OrderSentenceResult(
                    correctWord: "\(correctSentence.0) \(correctSentence.1)",
                    isCorrect: true
                )
            )
    ...
    func verifyNoOverlap() {
        let frames = [view1.frame, view2.frame]
    ...
    func goToResultScreen() {
        ...
        if let resultVC = storyboard.instantiateViewController(
            withIdentifier: "OrderSentenceResultLevel1VC"
        ) as? OrderSentenceResultLevel1VC {
    ...
    @IBAction func nextTapBtn(_ sender: UIButton) {
        ...
        if currentIndex < sentencePairs.count {
            loadCurrentWord()
            updateScoreLabel()
        } else {
            goToResultScreen()

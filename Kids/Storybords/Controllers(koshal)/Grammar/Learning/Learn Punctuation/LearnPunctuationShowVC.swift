...
class LearnPunctuationShowVC: BaseViewController,AVSpeechSynthesizerDelegate {
    ...
    func showSwipeUpHint() {
        ...
        let label = UILabel()
        label.text = "Swipe up to change punctuation".localiz()
    ...
    func restoreCardFromSmall(cardTag: Int) {
        ...
        // Jo punctuation already dikh chuke
        let viewedItems = Array(planets.prefix(currentIndex + 1))

        // Remaining ko random karo
    ...
    func goToResultScreen() {
        ...
        if let resultVC = storyboard.instantiateViewController(
            withIdentifier: "LearnPunctuationResultVC"
        ) as? LearnPunctuationResultVC {

//
//  WriteTimeHoursClockVC.swift
//  Kids
//
//  Created by Koshal Singh on 07/02/26.
//

struct WriteTimeHoursClockResult {
...
class WriteTimeHoursClockVC: UIView {
    ...
    private func updateHands() {
        ...
        minuteHand.transform =
            CGAffineTransform(rotationAngle: minuteAngle * .pi / 180)

        hourHand.transform =
            CGAffineTransform(rotationAngle: hourAngle * .pi / 180)
    }
    ...
    private func setupClock() {
        ...
        let minutePan = UIPanGestureRecognizer(target: self, action: #selector(handleMinutePan(_:)))
        ...
        let hourPan = UIPanGestureRecognizer(target: self, action: #selector(handleHourPan(_:)))

...
class WriteTimeMinutesVC: BaseViewController {
    ...
    func generateQuestions() {
        ...
        let possibleMinutes = [0, 15, 30, 45]
        
        for _ in 0..<10 {
            let hour = Int.random(in: 1...12)
            let minute = possibleMinutes.randomElement()!
            questions.append((hour, minute))
        }
    }
    ...
    func showResultScreen() {
        ...
        if let vc = storyboard.instantiateViewController(
            withIdentifier: "WriteTimeMinutesResultVC"
        ) as? WriteTimeMinutesResultVC {

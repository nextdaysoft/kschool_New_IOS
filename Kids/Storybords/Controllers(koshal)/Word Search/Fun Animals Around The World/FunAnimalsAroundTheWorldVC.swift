//
//  FunAnimalsAroundTheWorldVC.swift
//  Find Pair
//
...
class FunAnimalsAroundTheWorldVC: BaseViewController {
    ...
    @IBOutlet weak var nameView11: UIView!
    @IBOutlet weak var nameView12: UIView!
    ...
    let animals = [
     ...
     var animalItems: [AnimalLetterItem] = []
     var selectedAnimalItems: [AnimalLetterItem] = []
     var foundAnimals: Set<String> = []
     var lockedAnimalViews: Set<UIView> = []
     var animalIndexes: [String:[Int]] = [:]
     var animalToNameViewMap: [String:UIView] = [:]

     var formedAnimalWord = ""
    ...
    let customPurpleColor = UIColor(
     ...
     override func viewDidLoad() {
         ...
         nameView11.layer.cornerRadius = 6
         nameView11.backgroundColor = ColorManager.randomColor()
         nameView11.layer.borderColor = UIColor.black.cgColor
        

         nameView12.layer.cornerRadius = 6
         nameView12.backgroundColor = ColorManager.randomColor()
         nameView12.layer.borderColor = UIColor.black.cgColor
         ...
         scoreLabel.text = "\("Score".localiz()): 0 / \(animals.count)"
     }
    ...
    func setupGrid() {
        ...
        animalItems = zip(views, labels).compactMap {
            guard let v = $0.0, let l = $0.1 else { return nil }
            return AnimalLetterItem(view: v, label: l)
        }

        let letters = generateAnimalGridLetters()

        for i in 0..<animalItems.count {
            animalItems[i].label.text = letters[i]
            animalItems[i].view.backgroundColor = .white
            animalItems[i].label.textColor = .black
        }
    }
   
    // MARK: - SETUP
    func setupAnimalNames() {

        let labels = [
            nameLabel1,nameLabel2,nameLabel3,nameLabel4,nameLabel5,nameLabel6,
            nameLabel7,nameLabel8,nameLabel9,nameLabel10,nameLabel11,nameLabel12
        ]

        let views = [
            nameView1,nameView2,nameView3,nameView4,nameView5,nameView6,
            nameView7,nameView8,nameView9,nameView10,nameView11,nameView12
        ]
        ...
        for i in 0..<shuffled.count {
            ...
            animalToNameViewMap[shuffled[i]] = views[i]
        }
    }

    func setupGesture() {
        let pan = UIPanGestureRecognizer(target: self, action: #selector(handleAnimalPan(_:)))
        view.addGestureRecognizer(pan)
    }

    // MARK: - GRID LOGIC
    func generateAnimalGridLetters() -> [String] {

        let rows = 10
        let cols = 10
        var grid = Array(repeating: "", count: rows * cols)
        animalIndexes.removeAll()

        for animal in animals {

            let chars = Array(animal)
            var placed = false

            while !placed {

                let startRow = Int.random(in: 0..<rows)
                let startCol = Int.random(in: 0..<(cols - chars.count + 1))

                var indexes: [Int] = []
                var conflict = false

                for i in 0..<chars.count {
                    let index = startRow * cols + startCol + i
                ...
                for (i, idx) in indexes.enumerated() {
                    grid[idx] = String(chars[i])
                }

                animalIndexes[animal] = indexes
                placed = true
            }
        }
    ...
    // MARK: - PAN
    @objc func handleAnimalPan(_ gesture: UIPanGestureRecognizer) {

        let location = gesture.location(in: view)

        for item in animalItems {

            let point = item.view.convert(location, from: view)

            if item.view.bounds.contains(point),
               !selectedAnimalItems.contains(where: { $0.view == item.view }),
               !lockedAnimalViews.contains(item.view) {

                if currentColor == nil {
                    currentColor = customPurpleColor
                }

                selectedAnimalItems.append(item)
                formedAnimalWord += item.label.text ?? ""
                item.view.backgroundColor = currentColor!
    ...
    // MARK: - CHECK WORD
    func checkAnimalWord() {

        let word = formedAnimalWord.uppercased()
        let reversedWord = String(word.reversed())

        if (animals.contains(word) || animals.contains(reversedWord)),
           !(foundAnimals.contains(word) || foundAnimals.contains(reversedWord)) {

            let correctWord = animals.contains(word)
                ? word
                : reversedWord

            foundAnimals.insert(correctWord)

            let finalColor =
                colors[usedColorIndex % colors.count]

            selectedAnimalItems.forEach {
                lockedAnimalViews.insert($0.view)
                $0.view.backgroundColor = finalColor
                $0.label.textColor = .white
            }

            if let nameView = animalToNameViewMap[correctWord] {
                nameView.isHidden = true
            }

            usedColorIndex += 1
            scoreLabel.text = "\("Score".localiz()): \(foundAnimals.count) / \(animals.count)"
        ...
        formedAnimalWord = ""
        selectedAnimalItems.removeAll()
        currentColor = nil

        if foundAnimals.count == animals.count {

            didCompleteGame = true

            UserDefaults.standard.set(
                true,
                forKey: "wordSearchLevel2Completed"
            )
    ...
    @IBAction func hintAndNextTapBtn(_ sender: UIButton) {
        ...
        guard let animal = animals.first(where: { !foundAnimals.contains($0) }),
              let indexes = animalIndexes[animal],
              let firstIndex = indexes.first else { return }

        let item = animalItems[firstIndex]
        if !lockedAnimalViews.contains(item.view) {

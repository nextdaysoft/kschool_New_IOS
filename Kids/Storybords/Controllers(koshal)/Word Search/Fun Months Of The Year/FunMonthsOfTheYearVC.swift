...
class FunMonthsOfTheYearVC: BaseViewController {
    ...
    @IBOutlet weak var view101: UIView!
    @IBOutlet weak var view102: UIView!
    @IBOutlet weak var view103: UIView!
    @IBOutlet weak var view104: UIView!
    @IBOutlet weak var view105: UIView!
    @IBOutlet weak var view106: UIView!
    @IBOutlet weak var view107: UIView!
    @IBOutlet weak var view108: UIView!
    @IBOutlet weak var view109: UIView!
    @IBOutlet weak var view110: UIView!
    @IBOutlet weak var view111: UIView!
    @IBOutlet weak var view112: UIView!
    @IBOutlet weak var view113: UIView!
    @IBOutlet weak var view114: UIView!
    @IBOutlet weak var view115: UIView!
    @IBOutlet weak var view116: UIView!
    @IBOutlet weak var view117: UIView!
    @IBOutlet weak var view118: UIView!
    @IBOutlet weak var view119: UIView!
    @IBOutlet weak var view120: UIView!
    ...
    var monthItems: [MonthLetterItem] = []
    var selectedItems: [MonthLetterItem] = []
    var foundMonths: Set<String> = []
    var lockedViews: Set<UIView> = []
    var monthIndexes: [String:[Int]] = [:]
    ...
    override func viewDidLoad() {
        ...
        monthItems = zip(
            [view1,view2,view3,view4,view5,view6,view7,view8,view9,view10,
             ...
             view91,view92,view93,view94,view95,view96,view97,view98,view99,view100, view101, view102, view103, view104, view105,
             view106, view107, view108, view109, view110,
             view111, view112, view113, view114, view115,
             view116, view117, view118, view119, view120],
            [label1,label2,label3,label4,label5,label6,label7,label8,label9,label10,
             ...
             label91,label92,label93,label94,label95,label96,label97,label98,label99,label100, label101, label102, label103, label104, label105,
             label106, label107, label108, label109, label110,
             label111, label112, label113, label114, label115,
             label116, label117, label118, label119, label120]
        ).compactMap {
            guard let v = $0.0, let l = $0.1 else { return nil }
            return MonthLetterItem(view: v, label: l)
        }
        ...
        scoreLabel.text = "\("Score".localiz()): 0 / \(months.count)"
    }
    ...
    func setupGrid() {
        ...
        for i in 0..<monthItems.count {

            if i < letters.count {
                monthItems[i].label.text = letters[i]
            } else {
                monthItems[i].label.text = ""
            }

            monthItems[i].view.backgroundColor = .white
            monthItems[i].label.textColor = .black
        }
    }
    ...
    func generateGridLetters() -> [String] {
        ...
        var grid = Array(repeating: "", count: total)
        monthIndexes.removeAll()

        for month in months {

            let chars = Array(month)
            var placed = false
    ...
    @objc func handlePan(_ gesture: UIPanGestureRecognizer) {
        ...
        for item in monthItems {
            ...
            if item.view.bounds.contains(p),
               ...
               !lockedViews.contains(item.view) {
    ...
    func checkWord() {
        ...
        if (months.contains(word) || months.contains(reversedWord)),
           !(foundMonths.contains(word) || foundMonths.contains(reversedWord)) {

            let correctWord = months.contains(word)
                ? word
                : reversedWord

            foundMonths.insert(correctWord)

            let finalColor =
                colors[usedColorIndex % colors.count]

            selectedItems.forEach {
                lockedViews.insert($0.view)
            ...
            scoreLabel.text = "\("Score".localiz()): \(foundMonths.count) / \(months.count)"
        ...
        if foundMonths.count == months.count {

            didCompleteGame = true

            UserDefaults.standard.set(
                true,
                forKey: "wordSearchLevel3Completed"
            )
    ...
    func createPDF() -> URL? {
        ...
        let pdfURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("FunMonthsOfTheYear.pdf")
    ...
    @IBAction func hintAndNextTapBtn(_ sender: UIButton) {
        ...
        guard let currentMonth = months.first(where: { !foundMonths.contains($0) }),
              let indexes = monthIndexes[currentMonth],
              let firstIndex = indexes.first else { return }

        let item = monthItems[firstIndex]
        
        if !lockedViews.contains(item.view) {
            flashlockedItem(item, color: customPurpleColor)
        }
    }

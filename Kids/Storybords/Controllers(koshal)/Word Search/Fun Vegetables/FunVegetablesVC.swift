...
class FunVegetablesVC: BaseViewController {
    ...
    var vegetableItems: [FunVegetableLetterItem] = []
    var selectedVegetableItems: [FunVegetableLetterItem] = []
    var formedVegetableWord = ""
    var foundVegetableWords: Set<String> = []
    var lockedVegetableViews: Set<UIView> = []
    var vegetableWordIndexes: [String:[Int]] = [:]
    var vegetableToViewMap: [String:UIView] = [:]
    ...
    override func viewDidLoad() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(handleVegetablePan(_:)))
        view.addGestureRecognizer(pan)

        scoreLabel.text = "\("Score".localiz()): 0 / \(vegetablesList.count)"
    }
    ...
    func setupVegetableNames() {
        ...
        let shuffled = vegetablesList.shuffled()

        for i in 0..<vegetablesList.count {
            labels[i]?.text = shuffled[i]
            vegetableToViewMap[shuffled[i]] = views[i]
            views[i]?.layer.cornerRadius = 10
        }
    }

    // MARK: - SETUP GRID
    func setupVegetableGrid() {
        ...
        vegetableItems = zip(views, labels).compactMap {
            guard let v = $0.0, let l = $0.1 else { return nil }
            return FunVegetableLetterItem(view: v, label: l)
        }

        let letters = generateVegetableGridLetters()

        for (i,item) in vegetableItems.enumerated() {
    ...
    func generateVegetableGridLetters() -> [String] {

        let rows = 10, cols = 10
        var grid = Array(repeating: "", count: rows * cols)
        vegetableWordIndexes.removeAll()

        var startRow = 0

        for word in vegetablesList {
            ...
            vegetableWordIndexes[word] = indexes
            startRow += 1
        }
    ...
    // MARK: - PAN GESTURE
    @objc func handleVegetablePan(_ gesture: UIPanGestureRecognizer) {

        let loc = gesture.location(in: view)

        for item in vegetableItems {

            let p = item.view.convert(loc, from: view)

            if item.view.bounds.contains(p),
               !selectedVegetableItems.contains(where: {$0.view == item.view}),
               !lockedVegetableViews.contains(item.view) {

                if currentWordColor == nil {
                    currentWordColor = customPurpleColor
                }

                selectedVegetableItems.append(item)
                formedVegetableWord += item.label.text ?? ""
    ...
    // MARK: - CHECK WORD
    func checkVegetableWord() {

        let word = formedVegetableWord.uppercased()
        let reversedWord = String(word.reversed())

        if (vegetablesList.contains(word) || vegetablesList.contains(reversedWord)),
           !(foundVegetableWords.contains(word) || foundVegetableWords.contains(reversedWord)) {

            let correctWord = vegetablesList.contains(word)
                ? word
                : reversedWord

            foundVegetableWords.insert(correctWord)

            let finalColor =
                selectionColors[usedColorIndex % selectionColors.count]

            selectedVegetableItems.forEach {
                lockedVegetableViews.insert($0.view)
                $0.view.backgroundColor = finalColor
                $0.label.textColor = .white
            }

            vegetableToViewMap[correctWord]?.isHidden = true

            score += 1
            usedColorIndex += 1

            scoreLabel.text = "\("Score".localiz()): \(score) / \(vegetablesList.count)"
        ...
        formedVegetableWord = ""
        selectedVegetableItems.removeAll()
        currentWordColor = nil

        if foundVegetableWords.count == vegetablesList.count {

            didCompleteGame = true

            UserDefaults.standard.set(
                true,
                forKey: "wordSearchLevel9Completed"
            )
    ...
    @IBAction func hintAndNextTapBtn(_ sender: UIButton) {
        ...
        if didCompleteGame {
            if let menuVC = navigationController?.viewControllers.first(
                ...
                menuVC.level9Completed = true
            }
            navigationController?.popViewController(animated: true)
            return
        }

        guard let pendingWord = vegetablesList.first(where: { !foundVegetableWords.contains($0) }),
              let index = vegetableWordIndexes[pendingWord]?.first else { return }

        let item = vegetableItems[index]

        if !lockedVegetableViews.contains(item.view) {
            flashVegetableItem(item)
        }
    }

    func flashVegetableItem(_ item: FunVegetableLetterItem) {
        UIView.animate(withDuration: 0.25, animations: {
            item.view.backgroundColor = self.customPurpleColor
            item.label.textColor = .white
        }) { _ in
            UIView.animate(withDuration: 0.25, delay: 0.4) {
                if !self.lockedVegetableViews.contains(item.view) {
                    item.view.backgroundColor = .white
                    item.label.textColor = .black
                }
            }

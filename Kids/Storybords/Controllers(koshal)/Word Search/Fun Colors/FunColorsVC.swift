...
class FunColorsVC: BaseViewController {
    ...
    var letterItems: [FunColorLetterItem] = []
    var selectedItems: [FunColorLetterItem] = []
    var formedWord = ""
    var foundWords: Set<String> = []
    var lockedItems: Set<UIView> = []
    var wordIndexes: [String:[Int]] = [:]
    var colorToViewMap: [String:UIView] = [:]
    ...
    override func viewDidLoad() {
        ...
        scoreLabel.text = "\("Score".localiz()): 0 / \(colorsList.count)"
    }
    ...
    func setupNames() {
        ...
        let shuffled = colorsList.shuffled()

        for i in 0..<colorsList.count {
            labels[i]?.text = shuffled[i]
            colorToViewMap[shuffled[i]] = views[i]
    ...
    func setupGrid() {
        ...
        letterItems = zip(views, labels).compactMap {
            ...
            return FunColorLetterItem(view: v, label: l)
        }
    ...
    func checkWord() {
        ...
        if (colorsList.contains(word) || colorsList.contains(reversedWord)),
           !(foundWords.contains(word) || foundWords.contains(reversedWord)) {

            let correctWord = colorsList.contains(word)
                ? word
                : reversedWord
            ...
            colorToViewMap[correctWord]?.isHidden = true

            score += 1
            usedColorIndex += 1

            scoreLabel.text = "\("Score".localiz()): \(score) / \(colorsList.count)"
        ...
        if foundWords.count == colorsList.count {

            didCompleteGame = true

            UserDefaults.standard.set(
                true,
                forKey: "wordSearchLevel6Completed"
            )
    ...
    func flashItem(_ item: FunColorLetterItem, color: UIColor) {
    ...
    func createPDF() -> URL? {
        ...
        let pdfURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("FunColors.pdf")
    ...
    @IBAction func hintAndNextTapBtn(_ sender: UIButton) {
        ...
        guard let pendingWord = colorsList.first(where: { !foundWords.contains($0) }),
              let index = wordIndexes[pendingWord]?.first else { return }

        let item = letterItems[index]

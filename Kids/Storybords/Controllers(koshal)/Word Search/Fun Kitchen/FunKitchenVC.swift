...
class FunKitchenVC: BaseViewController {
    ...
    var kitchenItems: [FunKitchenLetterItem] = []
    var selectedKitchenItems: [FunKitchenLetterItem] = []
    var formedKitchenWord = ""
    var foundKitchenWords: Set<String> = []
    var lockedKitchenViews: Set<UIView> = []
    var kitchenWordIndexes: [String:[Int]] = [:]
    var kitchenToViewMap: [String:UIView] = [:]
    ...
    override func viewDidLoad() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(handleKitchenPan(_:)))
        view.addGestureRecognizer(pan)

        scoreLabel.text = "\("Score".localiz()): 0 / \(kitchenList.count)"
    }
    ...
    func setupKitchenNames() {
        ...
        let shuffled = kitchenList.shuffled()

        for i in 0..<kitchenList.count {
            labels[i]?.text = shuffled[i]
            kitchenToViewMap[shuffled[i]] = views[i]
            views[i]?.layer.cornerRadius = 10
        }
    }

    // MARK: - SETUP GRID
    func setupKitchenGrid() {
        ...
        kitchenItems = zip(views, labels).compactMap {
            guard let v = $0.0, let l = $0.1 else { return nil }
            return FunKitchenLetterItem(view: v, label: l)
        }

        let letters = generateKitchenGridLetters()

        for (i,item) in kitchenItems.enumerated() {
    ...
    func generateKitchenGridLetters() -> [String] {

        let rows = 10, cols = 10
        var grid = Array(repeating: "", count: rows * cols)
        kitchenWordIndexes.removeAll()

        var startRow = 0

        for word in kitchenList {
            ...
            kitchenWordIndexes[word] = indexes
            startRow += 1
        }
    ...
    // MARK: - PAN GESTURE
    @objc func handleKitchenPan(_ gesture: UIPanGestureRecognizer) {
        ...
        for item in kitchenItems {
            ...
            if item.view.bounds.contains(p),
               !selectedKitchenItems.contains(where: {$0.view == item.view}),
               !lockedKitchenViews.contains(item.view) {

                if currentSelectColor == nil {
                    currentSelectColor = customPurpleColor
                }

                selectedKitchenItems.append(item)
                formedKitchenWord += item.label.text ?? ""
    ...
    // MARK: - CHECK WORD
    func checkKitchenWord() {

        let word = formedKitchenWord.uppercased()
        let reversedWord = String(word.reversed())

        if (kitchenList.contains(word) || kitchenList.contains(reversedWord)),
           !(foundKitchenWords.contains(word) || foundKitchenWords.contains(reversedWord)) {

            let correctWord = kitchenList.contains(word)
                ? word
                : reversedWord

            foundKitchenWords.insert(correctWord)
            ...
            selectedKitchenItems.forEach {
                lockedKitchenViews.insert($0.view)
                $0.view.backgroundColor = finalColor
                $0.label.textColor = .white
            }

            kitchenToViewMap[correctWord]?.isHidden = true

            score += 1
            usedColorIndex += 1

            scoreLabel.text = "\("Score".localiz()): \(score) / \(kitchenList.count)"
        ...
        formedKitchenWord = ""
        selectedKitchenItems.removeAll()
        currentSelectColor = nil

        if foundKitchenWords.count == kitchenList.count {

            didCompleteGame = true

            UserDefaults.standard.set(
                true,
                forKey: "wordSearchLevel8Completed"
            )
    ...
    @IBAction func hintAndNextTapBtn(_ sender: UIButton) {
        ...
        if didCompleteGame {
            if let menuVC = navigationController?.viewControllers.first(
                ...
                menuVC.level8Completed = true
            }
            navigationController?.popViewController(animated: true)
            return
        }

        guard let pendingWord = kitchenList.first(where: { !foundKitchenWords.contains($0) }),
              let index = kitchenWordIndexes[pendingWord]?.first else { return }

        let item = kitchenItems[index]

        if !lockedKitchenViews.contains(item.view) {
            flashKitchenItem(item)
        }
    }

    func flashKitchenItem(_ item: FunKitchenLetterItem) {
        UIView.animate(withDuration: 0.25, animations: {
            item.view.backgroundColor = self.customPurpleColor
            item.label.textColor = .white
        }) { _ in
            UIView.animate(withDuration: 0.25, delay: 0.4) {
                if !self.lockedKitchenViews.contains(item.view) {
                    item.view.backgroundColor = .white
                    item.label.textColor = .black
                }
            }
    ...
    func createPDF() -> URL? {
        ...
        let pdfURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("FunKitchen.pdf")

//
//  GuessAnimalFamiliesResultVC.swift
//  KSchool
//
...
class GuessAnimalFamiliesResultVC: BaseViewController {
    ...
    func goToMenu(markCompleted: Bool = true) {
        ...
        if markCompleted {
            ...
            let completedKey = "animalFamiliesLevel\(levelNumber)Completed"
            let percentageKey = "animalFamiliesLevel\(levelNumber)Percentage"
        ...
        if let menuVC = navigationController?.viewControllers.first(
            where: { $0 is AnimalFamiliesMenuVC }
        ) {
